...
def test_criar_e_listar_por_usuario(db_session):
    ...
    repo.criar(
        usuario_id=u.id, fundo_id=f.id, quantidade=10, preco_medio=Decimal("100.00"), valor_investido=Decimal("1000.00")
    )
...
def test_buscar_filtra_por_usuario(db_session):
    ...
    repo = PosicaoRepository(db_session)
    p = repo.criar(
        usuario_id=u.id, fundo_id=f.id, quantidade=1, preco_medio=Decimal("10.00"), valor_investido=Decimal("10.00")
    )
